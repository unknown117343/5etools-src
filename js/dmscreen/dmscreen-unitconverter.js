...
export class UnitConverter extends DmScreenPanelAppBase {
	...
	_getPanelElement (board, state) {
		...
		const wrpConverter = ee`<div class="dm-unitconv dm__panel-bg split-column"></div>`;

		const tblConvert = ee`<table class="w-100 table-striped"></table>`.appendTo(wrpConverter);
		const tbodyConvert = ee`<tbody></tbody>`.appendTo(tblConvert);
		units.forEach((u, i) => {
			const eleTr = ee`<tr class="row clickable"></tr>`.appendTo(tbodyConvert);
			const clickL = () => {
				this._ixConv = i;
				this._dirConv = 0;
				updateDisplay();
			...
			ee`<td class="ve-col-3 code">×${u.x1.padStart(5)}</td>`.onn("click", evt => clickL(evt)).appendTo(eleTr);
			ee`<td class="ve-col-3">${u.n2}</td>`.onn("click", evt => clickR(evt)).appendTo(eleTr);
			ee`<td class="ve-col-3 code">×${u.x2.padStart(5)}</td>`.onn("click", evt => clickR(evt)).appendTo(eleTr);
		});

		const wrpIpt = ee`<div class="ve-flex dm-unitconv__wrp-ipt"></div>`.appendTo(wrpConverter);

		const wrpLeft = ee`<div class="split-column dm-unitconv__wrp-ipt-inner w-100"></div>`.appendTo(wrpIpt);
		const eleLblLeft = ee`<span class="bold"></span>`.appendTo(wrpLeft);
		this._iptLeft = ee`<textarea class="dm-unitconv__ipt form-control h-100">${state.i || ""}</textarea>`.appendTo(wrpLeft);
		...
		const wrpRight = ee`<div class="split-column dm-unitconv__wrp-ipt-inner w-100"></div>`.appendTo(wrpIpt);
		const eleLblRight = ee`<span class="bold"></span>`.appendTo(wrpRight);
		const iptRight = ee`<textarea class="dm-unitconv__ipt form-control h-100" disabled style="background: #0000"></textarea>`.appendTo(wrpRight);
